...
impl Dither for BurkesDither {
    fn ditherize<'a, T: crate::generic_image::GenericImage>(
        image: &'a T,
        palette: &[T::Pixel],
    ) -> &'a T {
        return dither_burkes(image, palette);
    }
}

pub fn dither_burkes<I, P, S>(image: &I, palette: &[I::Pixel]) -> ImageBuffer<P, Vec<S>>
where
    ...
    S: PixelComponent,
{
    ...
    for (x, y, v) in image.into_iter() {
        ...
        diffuse_error!(out, x + 1, y, 8 / 32);
        diffuse_error!(out, x + 2, y, 4 / 32);
        diffuse_error!(out, x - 2, y + 1, 2 / 32);
        diffuse_error!(out, x - 1, y + 1, 4 / 32);
        diffuse_error!(out, x, y + 1, 8 / 32);
        diffuse_error!(out, x + 1, y + 1, 4 / 32);
        diffuse_error!(out, x + 2, y + 1, 2 / 32);
    }
